...
<x-ui.dropdown-menu>
    ...
    <x-ui.dropdown-menu-content align="end" class="w-48">
        ...
        @if(function_exists('tenant') && tenant())
            <x-ui.dropdown-menu-item wire:click="goTo('{{ route('tenant.profile') }}')">
                <x-lucide-user class="mr-2 size-4" />
                <span>Profile</span>
            </x-ui.dropdown-menu-item>
        @else
            <x-ui.dropdown-menu-item wire:click="goTo('{{ route('central.account') }}')">
                <x-lucide-user class="mr-2 size-4" />
                <span>Profile</span>
            </x-ui.dropdown-menu-item>
            @if(Route::has('central.billing'))
                <x-ui.dropdown-menu-item wire:click="goTo('{{ route('central.billing') }}')">
