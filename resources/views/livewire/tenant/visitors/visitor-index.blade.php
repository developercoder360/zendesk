    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-xs text-muted-foreground mt-0.5">Real-time active website visitors, browsing sessions, and chat assignments.</p>
            </div>
            <div class="flex items-center space-x-4">
                <div class="relative">
                    <x-lucide-search class="absolute left-2.5 top-2.5 h-4 w-4 text-muted-foreground" />
                    <x-ui.input type="search" placeholder="Search visitors, IP, referrer..." class="pl-8 w-72" wire:model.live.debounce.300ms="search" />
                </div>
            </div>
        </div>
    </x-slot>
                        ...
                        <x-ui.table-body>
                            @forelse ($visitors as $visitor)
                                <x-ui.table-row class="hover:bg-muted/40 transition-colors">
                                    ...
                                    <x-ui.table-cell>
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if($visitor->is_banned)
                                                <span class="size-2.5 rounded-full bg-destructive shrink-0" title="Banned"></span>
                                            ...
                                            @else
                                                <span class="size-2.5 rounded-full bg-muted-foreground/40 shrink-0" title="Offline"></span>
                                            @endif
                                            ...
                                            @if($visitor->country_code)
                                                <span class="text-xs font-semibold px-1 py-0.5 rounded bg-muted text-muted-foreground uppercase" title="Country: {{ $visitor->country_code }}">
                                    ...
                                    <!-- Visitor Info -->
                                    <x-ui.table-cell>
                                        <div class="font-medium text-foreground">{{ $visitor->name ?: 'Anonymous Visitor' }}</div>
                                        <div class="text-xs text-muted-foreground flex items-center space-x-2 mt-0.5">
                                            <span>{{ $visitor->email ?: 'No email' }}</span>
                                            <span>•</span>
                                            <code class="font-mono text-[11px] text-muted-foreground/80">{{ $visitor->ip_address ?: 'Unknown IP' }}</code>
                                        </div>
                                    </x-ui.table-cell>
                                    ...
                                    <x-ui.table-cell>
                                        @if($visitor->servedByAgent && $visitor->servedByAgent->user)
                                            <div class="flex items-center space-x-2">
                                                <div class="size-6 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">
                                                    {{ substr($visitor->servedByAgent->user->name, 0, 1) }}
                                                </div>
                                                <span class="text-xs font-medium">{{ $visitor->servedByAgent->user->name }}</span>
                                            </div>
                                        @else
                                    ...
                                    <x-ui.table-cell class="max-w-xs">
                                        <div class="flex items-center space-x-1.5">
                                            <!-- Open Tabs Badge -->
                                            <span class="inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-primary/10 text-primary shrink-0" title="{{ $visitor->open_tabs_count ?? 1 }} open tab(s)">
                                                {{ $visitor->open_tabs_count ?? 1 }}
                                            </span>

                                            <!-- Page Title / URL -->
                                            <div class="truncate text-xs">
                                                <div class="font-medium truncate text-foreground" title="{{ $visitor->current_page_title ?: 'Viewing site' }}">
                                                    {{ $visitor->current_page_title ?: ($visitor->current_page_url ?: 'Home Page') }}
                                                </div>
                                                @if($visitor->current_page_url)
                                                    <a href="{{ $visitor->current_page_url }}" target="_blank" class="text-[11px] text-muted-foreground hover:underline truncate block">
                                    ...
                                    <x-ui.table-cell class="text-center">
                                        <div class="inline-flex items-center space-x-2 text-xs">
                                            <span class="px-2 py-0.5 rounded bg-muted text-muted-foreground font-medium" title="Visits count">
                                                {{ $visitor->visits_count ?? 1 }} visits
                                            </span>
                                            <span class="px-2 py-0.5 rounded bg-primary/10 text-primary font-medium" title="Chats count">
                                    ...
                                    <x-ui.table-cell class="text-right">
                                        @if($visitor->is_banned)
                                            <x-ui.button variant="outline" size="xs" wire:click="unbanVisitor('{{ $visitor->id }}')">
                                                Unban
                                            </x-ui.button>
                                        @else
                                            <x-ui.button variant="destructive" size="xs" wire:click="openBanModal('{{ $visitor->id }}')">
                                                Ban
                                            </x-ui.button>
                                        @endif
                                    </x-ui.table-cell>
        ...
        <x-ui.dialog-content>
            <x-ui.dialog-header>
                ...
                <x-ui.dialog-description>
                    Are you sure you want to ban this visitor? They will no longer be able to access the chat widget or submit tickets.
                </x-ui.dialog-description>
            </x-ui.dialog-header>
