<div class="space-y-6 max-w-6xl mx-auto sm:px-6 lg:px-8">
    <nav class="flex items-center text-sm text-muted-foreground mb-4">
        <span class="font-medium text-foreground">Agents</span>
    </nav>
    ...
    <x-ui.card>
        <div class="p-4 border-b flex flex-col sm:flex-row gap-4">
            <div class="relative flex-1">
                ...
                <x-ui.input wire:model.live.debounce.300ms="search" placeholder="Search agents by name or email..." class="pl-9 w-full max-w-sm" />
            </div>
            ...
            <x-ui.select wire:model.live="roleFilter" class="sm:w-[180px]">
                ...
                <x-ui.select-content>
                    ...
                    <x-ui.select-item value="Company Admin">Company Admin</x-ui.select-item>
            ...
            <table class="w-full caption-bottom text-sm">
                <thead class="[&_tr]:border-b">
                    <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Agent</th>
                        ...
                        <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Presence</th>
                        <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Open Chats</th>
                ...
                <tbody class="[&_tr:last-child]:border-0">
                    @forelse($users as $tenantUser)
                        <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                            <td class="p-4 align-middle">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-medium">{{ $tenantUser->user->name ?? 'Unknown Agent' }}</span>
                                        @if($tenantUser->position)
                                            <span class="text-xs text-muted-foreground">{{ $tenantUser->position }} &bull; {{ $tenantUser->user->email ?? '' }}</span>
                                        @else
                            ...
                            <td class="p-4 align-middle">
                                @if($roleName = $tenantUser->user?->roles->first()?->name)
                                    <span class="font-medium text-foreground">{{ $roleName }}</span>
                                @else
                                    <x-ui.badge variant="outline" class="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/50 dark:text-amber-400 dark:border-amber-800 font-normal">
                                        No role assigned
                                    </x-ui.badge>
                                @endif
                            </td>
                            <td class="p-4 align-middle text-muted-foreground">
                                {{ $tenantUser->department->name ?? 'Unassigned' }}
                            </td>
                            <td class="p-4 align-middle">
                                <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border @if(($tenantUser->status ?? 'offline') === 'online') bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/50 dark:text-emerald-400 dark:border-emerald-800 @elseif(($tenantUser->status ?? 'offline') === 'away') bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/50 dark:text-amber-400 dark:border-amber-800 @else bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-900/50 dark:text-slate-400 dark:border-slate-800 @endif">
                                    <span class="size-1.5 rounded-full @if(($tenantUser->status ?? 'offline') === 'online') bg-emerald-500 @elseif(($tenantUser->status ?? 'offline') === 'away') bg-amber-500 @else bg-slate-400 @endif"></span>
                                    {{ ucfirst($tenantUser->status ?? 'offline') }}
                                </div>
                            </td>
                            <td class="p-4 align-middle">
                                <x-ui.badge variant="secondary" class="font-normal">
                                    {{ $tenantUser->assigned_chats_count ?? 0 }} open
                                </x-ui.badge>
                            </td>
                            <td class="p-4 align-middle">
                            ...
                            <td class="p-4 align-middle text-muted-foreground whitespace-nowrap">
                                {{ $tenantUser->joined_at ? $tenantUser->joined_at->format('M d, Y') : ($tenantUser->created_at ? $tenantUser->created_at->format('M d, Y') : '—') }}
                            </td>
        ...
        <div class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg sm:rounded-lg">
            <div class="flex flex-col space-y-1.5 text-center sm:text-left">
                <h2 class="text-lg font-semibold leading-none tracking-tight">
                    {{ $showCreateModal ? 'Add New Agent' : 'Edit Agent' }}
                </h2>
            ...
            <form wire:submit="{{ $showCreateModal ? 'save' : 'update' }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">
                            Password 
                            @if($showEditModal)
                                <span class="text-xs font-normal text-muted-foreground">(leave blank to keep)</span>
                            @else
                                *
                            @endif
                        </label>
                        @if($showCreateModal)
                            <x-ui.input type="password" wire:model="form.password" required />
                        @else
                            <x-ui.input type="password" wire:model="form.password" />
                        @endif
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Department</label>
                        <x-ui.select wire:model="form.department_id">
                            <x-ui.select-trigger>
                        ...
                        @error('form.department_id') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Presence Status</label>
                        <x-ui.select wire:model="form.status">
                            <x-ui.select-trigger>
                                <x-ui.select-value placeholder="Select Status" />
                            </x-ui.select-trigger>
                            <x-ui.select-content>
                                <x-ui.select-item value="offline">Offline</x-ui.select-item>
                                <x-ui.select-item value="online">Online</x-ui.select-item>
                                <x-ui.select-item value="away">Away</x-ui.select-item>
                            </x-ui.select-content>
                        </x-ui.select>
                        @error('form.status') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <x-ui.input wire:model="form.position" placeholder="e.g. Support Lead" />
                    ...
                    <div class="space-y-2">
                        ...
                        <x-ui.input wire:model="form.phone" placeholder="+1 (555) 000-0000" />
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Shift Schedule</label>
                        <x-ui.input wire:model="form.shift" placeholder="e.g. Morning (9am - 5pm)" />
                        @error('form.shift') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Avatar URL</label>
                        <x-ui.input wire:model="form.avatar" placeholder="https://example.com/avatar.jpg" />
                        @error('form.avatar') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-4">
                    ...
                    <x-ui.button type="submit">
                        {{ $showCreateModal ? 'Create Agent' : 'Save Changes' }}
                    </x-ui.button>
                </div>
            </form>
        </div>
    ...
    <div class="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm transition-all duration-100">
        <div class="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg sm:rounded-lg">
            <div class="flex flex-col space-y-1.5 text-center sm:text-left">
                ...
                @if($tenantUserToDelete && $tenantUserToDelete->assigned_chats_count > 0)
                    <div class="rounded-md bg-amber-50 p-3 text-xs text-amber-800 dark:bg-amber-950/50 dark:text-amber-300 border border-amber-200 dark:border-amber-800 flex items-start gap-2 mt-2">
                        <x-lucide-alert-triangle class="size-4 text-amber-600 shrink-0 mt-0.5" />
                        <span>This agent has <strong>{{ $tenantUserToDelete->assigned_chats_count }}</strong> active open chat(s) that will become unassigned if you continue.</span>
                    </div>
                @endif
            </div>
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-4">
                ...
                <x-ui.button variant="destructive" wire:click="delete">Delete Agent</x-ui.button>
            </div>
        </div>
    </div>
    @endif
