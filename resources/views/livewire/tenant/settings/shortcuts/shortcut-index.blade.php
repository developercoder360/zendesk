<div>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-foreground leading-tight">Shortcuts</h2>
            <div class="flex items-center space-x-2">
                <button class="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-colors" title="View options">
                    <x-lucide-layout-grid class="size-4" />
                </button>
                <button class="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-colors" title="Your profile">
                    <div class="size-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    >
                        <div class="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Availability</div>
                        <button
                            wire:click="$set('filterAgent', 'all')"
                            @click="open = false"
                        ...
                        >
                            @if($filterAgent === 'all') <x-lucide-check class="size-3.5" /> @else <span class="size-3.5"></span> @endif
                            All shortcuts
                        </button>
                        <button
                            wire:click="$set('filterAgent', 'global')"
                            @click="open = false"
                        ...
                        >
                            @if($filterAgent === 'global') <x-lucide-check class="size-3.5" /> @else <span class="size-3.5"></span> @endif
                            All agents (global)
                        </button>
                        @if($agents->count())
                            <div class="border-t border-border my-1"></div>
                            <div class="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">By Agent</div>
                            @foreach($agents as $agent)
                                <button
                                    wire:click="$set('filterAgent', '{{ $agent->id }}')"
                                    @click="open = false"
                                ...
                                >
                                    @if($filterAgent == $agent->id) <x-lucide-check class="size-3.5" /> @else <span class="size-3.5"></span> @endif
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-border bg-muted/40">
                                ...
                                <th class="px-4 py-3 text-left font-semibold text-foreground">
                                    <button wire:click="sortBy('shortcut_key')" class="inline-flex items-center gap-1 hover:text-primary transition-colors group">
                                        Shortcut
                                ...
                                <!-- Message -->
                                <th class="px-4 py-3 text-left font-semibold text-foreground">
                                    <button wire:click="sortBy('title')" class="inline-flex items-center gap-1 hover:text-primary transition-colors group">
                                        Message
                                        <span class="inline-flex flex-col text-muted-foreground group-hover:text-primary">
                                            @if($sortField === 'title')
                                                @if($sortDirection === 'asc')
                        ...
                        <tbody class="divide-y divide-border">
                            @forelse ($shortcuts as $item)
                                <tr
                                    class="hover:bg-muted/30 transition-colors cursor-pointer {{ in_array((string)$item->id, $selectedIds) ? 'bg-primary/5' : '' }}"
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3.5">
                                        <code class="inline-flex items-center px-2 py-0.5 rounded bg-muted text-foreground font-mono text-xs font-semibold">
                                            /{{ $item->shortcut_key }}
                                        </code>
                                    </td>
                                    ...
                                    <td class="px-4 py-3.5 max-w-md">
                                        <div class="font-medium text-foreground text-sm mb-0.5">{{ $item->title }}</div>
                                    ...
                                    <td class="px-4 py-3.5 text-sm text-muted-foreground whitespace-nowrap">
                                        ...
                                        @elseif($item->tenantUser && $item->tenantUser->user)
                                            <div class="flex items-center gap-1.5">
                                                <div class="size-5 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[10px] font-bold shrink-0">
                                    ...
                                    <!-- Tags (no tagging system yet — leave empty with placeholder) -->
                                    <td class="px-4 py-3.5">
                ...
                @if($shortcuts->hasPages())
                    <div class="px-4 py-3 border-t border-border flex items-center justify-between">
                        <p class="text-xs text-muted-foreground">
                            Showing {{ $shortcuts->firstItem() }}–{{ $shortcuts->lastItem() }} of {{ $shortcuts->total() }}
                        </p>
                        {{ $shortcuts->links() }}
                    </div>
                @endif
            ...
            <form wire:submit="save">
                <div class="grid gap-4 py-4">
                    ...
                    <div class="grid gap-2">
                        ...
                        @error('title') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>

                    <div class="grid gap-2">
                        <x-ui.label for="modal-shortcut">Shortcut Trigger</x-ui.label>
                        <div class="flex items-center">
                            <span class="inline-flex items-center bg-muted px-3 h-9 border border-r-0 border-input rounded-l-md text-sm text-muted-foreground font-mono">/</span>
                            <x-ui.input id="modal-shortcut" wire:model="shortcut" class="rounded-l-none" placeholder="greet" />
                        </div>
                        @error('shortcut') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>

                    <div class="grid gap-2">
                        <x-ui.label for="modal-content">Response Content</x-ui.label>
                        <x-ui.textarea id="modal-content" wire:model="content" rows="5" placeholder="Hi there, how can I help you today?" />
                        @error('content') <span class="text-xs text-destructive">{{ $message }}</span> @enderror
                    </div>

                    <div class="flex items-start space-x-2.5 pt-1">
                        <input type="checkbox" id="modal-isShared" wire:model="isShared" class="mt-0.5 rounded border-input size-4 accent-primary cursor-pointer" />
                        <div>
                            <label for="modal-isShared" class="text-sm font-medium cursor-pointer">Share with all agents</label>
                            <p class="text-xs text-muted-foreground mt-0.5">When checked, this shortcut is available to the whole team (global). Otherwise it's private to you.</p>
                        </div>
                    </div>
