<div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
    <nav class="flex items-center text-sm text-muted-foreground mb-4">
        ...
        <span class="font-medium text-foreground">Ticket #{{ $ticket->id }}</span>
    </nav>
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Ticket #{{ $ticket->id }}: Chat from
                {{ $ticket->visitor->name ?? 'Unknown' }}</h1>
            <p class="text-sm text-muted-foreground">View and respond to the ticket.</p>
        </div>
            ...
            <x-ui.card>
                ...
                <x-ui.card-content>
                    <form wire:submit.prevent="addReply" class="space-y-4">
                        <div>
                            <textarea wire:model="replyBody" rows="4" placeholder="Type your reply here..." required
                        ...
                        <x-ui.select id="status" wire:model.live="status">
                            ...
                            <x-ui.select-content>
                                ...
                                @foreach ($statuses as $st)
                                    <x-ui.select-item
                                        value="{{ $st->id }}">{{ $st->name }}</x-ui.select-item>
                                @endforeach
                            </x-ui.select-content>
                        </x-ui.select>
                    </div>
                        ...
                        <x-ui.select id="agent" wire:model.live="assigned_agent_id">
                            ...
                            <x-ui.select-content>
                                ...
                                @foreach ($agents as $agent)
                                    <x-ui.select-item
                                        value="{{ $agent->id }}">{{ $agent->user->name ?? 'Unknown' }}</x-ui.select-item>
                                @endforeach
                            </x-ui.select-content>
                        </x-ui.select>
                    </div>
                        ...
                        <x-ui.select id="department" wire:model.live="department_id">
                            ...
                            <x-ui.select-content>
                                ...
                                @foreach ($departments as $dept)
                                    <x-ui.select-item
                                        value="{{ $dept->id }}">{{ $dept->name }}</x-ui.select-item>
                                @endforeach
                            </x-ui.select-content>
                        </x-ui.select>
                    </div>
