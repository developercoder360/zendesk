        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                <div class="relative">
                    ...
                    <x-ui.input type="search" placeholder="Search history..." class="pl-8 w-64" wire:model.live.debounce.300ms="search" />
                </div>
                    ...
                    <x-ui.table>
                        ...
                        <x-ui.table-body>
                            @forelse ($tickets as $ticket)
                                <x-ui.table-row class="cursor-pointer hover:bg-muted/50" wire:click="$navigate('{{ route('tenant.tickets.show', $ticket) }}')">
                                    <x-ui.table-cell>
                                        ...
                                        @if($ticket->agent && $ticket->agent->user)
                                            <div class="flex items-center gap-2">
                                                <x-ui.avatar class="h-6 w-6">
                                                    <x-ui.avatar-fallback class="text-xs">{{ substr($ticket->agent->user->name, 0, 2) }}</x-ui.avatar-fallback>
                                                </x-ui.avatar>
